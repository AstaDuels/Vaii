--[[
    ASTA REMAKE – Full Hub (Simplificado)
    ...
    GRAB CARRY (Auto Carry Speed) - Ativa automaticamente ao carregar brainrot.
]]
...
-- ============================================================
-- GRAB CARRY (Auto Carry Speed) - SISTEMA NOVO
-- ============================================================
local autoCarrySpeedEnabled = false  -- Toggle principal
local grabCarryActive = false        -- Estado interno (se está ativo via detecção)
local grabCarryTimestamp = 0
local grabCarryGrace = 0.75          -- Tempo de graça após soltar
local grabCarryPreviousMode = "Normal"

-- Função para detectar se está carregando um brainrot/animal
local function isCarryingBrainrot(char)
    ...
    -- Verifica se há um modelo com Humanoid (animal carregado)
    for _, child in ipairs(char:GetChildren()) do
        ...
        -- Verifica tools com nome de brainrot
...
-- Função para ativar o Grab Carry
local function activateGrabCarry()
    if not autoCarrySpeedEnabled then return end
    if grabCarryActive then return end
    
    -- Salva o modo anterior
    if laggerModeEnabled then
        grabCarryPreviousMode = "Lagger"
    elseif carrySpeedActive then
        grabCarryPreviousMode = "Carry"
    else
        grabCarryPreviousMode = "Normal"
    end
    
    grabCarryActive = true
    grabCarryTimestamp = tick()
...
-- Função para desativar o Grab Carry (volta ao modo anterior)
local function deactivateGrabCarry()
    if not grabCarryActive then return end
    grabCarryActive = false
    
    -- Volta ao modo anterior
    if grabCarryPreviousMode == "Lagger" then
        laggerModeEnabled = true
        carrySpeedActive = false
    elseif grabCarryPreviousMode == "Carry" then
        carrySpeedActive = true
...
-- Loop de monitoramento do Grab Carry
task.spawn(function()
    while true do
        task.wait(0.1)
        
        if not autoCarrySpeedEnabled then
            if grabCarryActive then
                deactivateGrabCarry()
            end
            goto continue
        end
        ...
        if not char then
            if grabCarryActive then deactivateGrabCarry() end
            goto continue
        end
        ...
        if carrying or isStealing then
            -- Está carregando → ativa Grab Carry
            activateGrabCarry()
        elseif grabCarryActive then
            -- Não está mais carregando → espera o tempo de graça
            if tick() - grabCarryTimestamp > grabCarryGrace then
                deactivateGrabCarry()
            end
        end
...
local function buildMobileButtons()
    ...
    local buttonOrder = {
        {key="grabCarry",  label="GRAB CARRY",  toggle=true},  -- NOVO BOTÃO
    ...
    local function makeButton(def, index)
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            if def.key == "grabCarry" then
                autoCarrySpeedEnabled = not autoCarrySpeedEnabled
                if not autoCarrySpeedEnabled and grabCarryActive then
                    deactivateGrabCarry()
                end
                setOn(autoCarrySpeedEnabled)
                saveConfig(); return
            end
    ...
    if mobBtnRefs.grabCarry then mobBtnRefs.grabCarry(autoCarrySpeedEnabled) end
    if mobBtnRefs.autoLeft then mobBtnRefs.autoLeft(autoLeftEnabled) end
    if mobBtnRefs.autoRight then mobBtnRefs.autoRight(autoRightEnabled) end
    if mobBtnRefs.carrySpeed then mobBtnRefs.carrySpeed(carrySpeedActive) end
    if mobBtnRefs.lagger then mobBtnRefs.lagger(laggerModeEnabled) end
...
-- ============================================================
-- SAVE CONFIG (atualizado com Grab Carry)
-- ============================================================
saveConfig = function()
    local function ks(e)
    ...
    local cfg = {
        ...
        autoCarrySpeedEnabled = autoCarrySpeedEnabled,  -- NOVO
    }
...
local function resetAllSettings()
    ...
    batV2Enabled = false
    autoCarrySpeedEnabled = false  -- NOVO
    ...
    if mobBtnRefs.tpLock then mobBtnRefs.tpLock(bypassTPEnabled) end
    if mobBtnRefs.batV2 then mobBtnRefs.batV2(batV2Enabled) end
    if mobBtnRefs.grabCarry then mobBtnRefs.grabCarry(false) end
...
-- ============================================================
-- FULL CONFIG LOAD (incluindo resetKeybind, dropEnabled, batV2Enabled, autoCarrySpeedEnabled)
-- ============================================================
pcall(function()
    ...
    if type(d.autoCarrySpeedEnabled)=="boolean" then autoCarrySpeedEnabled=d.autoCarrySpeedEnabled end  -- NOVO
end)
...
;(function()
    ...
    -- NOVO: GRAB CARRY TOGGLE
    addSectLbl(sp,"GRAB CARRY (Auto)",9)
    addToggleRow(sp,"Grab Carry (auto ativa)",autoCarrySpeedEnabled,10,nil,function(on)
        autoCarrySpeedEnabled = on
        if not on and grabCarryActive then
            deactivateGrabCarry()
        end
        if mobBtnRefs.grabCarry then mobBtnRefs.grabCarry(on) end
        saveConfig()
    end)
end)()
...
if autoCarrySpeedEnabled and grabCarryActive then activateGrabCarry() end  -- NOVO
CandyApplyCustomSky(currentSkyTheme)
buildMobileButtons()

end)() -- end GUI function

print("✅ ASTA REMAKE LOADED – Anti-Kick 100% robusto + Bat V2 (tecla B) + Grab Carry (auto ativa ao carregar)")
